...
class ProductMultipleReference(models.Model):
    ...
    _description = 'Multi-Referencia por producto'
    _rec_name = 'multiple_references_name'

    multiple_references_name = fields.Char(string='Multi-Referencia', required=True)
    product_id = fields.Many2one('product.product', string="Producto", required=True)
    is_default_reference = fields.Boolean(string="Es Referencia por defecto", compute='_is_default_reference')
    ...
    def create(self, values):
        if values.get('multiple_references_name') and values.get('product_id'):
            ...
            if reference_code:
                ...
                res = super(ProductMultipleReference, self).create(values)
    ...
    def write(self, values):
        ...
        return super(ProductMultipleReference, self).write(values)


class ProductProduct(models.Model):
    _inherit = 'product.product'

    multiple_references_ids = fields.One2many("product.multiple.reference", "product_id", string="Multi-Referencias")
    multiple_references_code = fields.Char(string="Código Multi-Referencia", related="multiple_references_ids.multiple_references_name")
    multiple_references_id = fields.Many2many("product.multiple.reference", string="Multi-Referencia", compute="_get_multiple_reference")
    multiple_references_count = fields.Integer(string="Nro de Referencias", compute="_get_multiple_reference_count")
    ...
    def write(self, values):
        if values.get('default_code'):
            if self.default_code:
                self.env['product.multiple.reference'].sudo().create_reference(self.default_code, self.id)
        # print(values)
...
class ProductTemplate(models.Model):
    ...
    multiple_references_code = fields.Char(string="Multiple Referencias", 
        related="product_variant_ids.multiple_references_ids.multiple_references_name")
    multiple_references_id = fields.Many2many("product.multiple.reference", string="Multiple Referencias", compute="_get_multiple_reference")
    multiple_references_count = fields.Integer(string="Nro de Referencias", compute="_get_multiple_reference_count")
