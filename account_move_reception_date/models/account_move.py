...
class AccountMove(models.Model):
    ...
    invoice_reception_date = fields.Date(string='Fecha de recepción',
                                         states={'draft': [('readonly', False)],'posted': [('readonly', True)]})

    # Sobreescribe para que el calculo de la fecha de vencimiento sea sobre
    # la fecha de recepcion y no la de la fecha de emision
    @api.onchange('invoice_reception_date','invoice_date')
    def _onchange_invoice_reception_date(self):
        if self.invoice_reception_date and self.invoice_date:
            if self.invoice_reception_date >= self.invoice_date:
                if not self.invoice_payment_term_id and (
                        not self.invoice_date_due or self.invoice_date_due < self.invoice_reception_date):
                    self.invoice_date_due = self.invoice_reception_date
                #                 self.invoice_date = self.invoice_reception_date
                if self.date != self.invoice_reception_date:  # Don't flag date as dirty if not needed
                    self.date = self.invoice_reception_date
    ...
    def _recompute_dynamic_lines(self, recompute_all_taxes=False, recompute_tax_base_amount=False):
        ...
        for invoice in self:
            ...
            if invoice.is_invoice(include_receipts=True):
                ...
                invoice._recompute_payment_terms_lines_extend()
    ...
    def _recompute_payment_terms_lines_extend(self):
        ...
        def _get_payment_terms_computation_date(self):
            ...
            if self.invoice_payment_term_id:
                return self.invoice_reception_date or self.invoice_date or today
            else:
                return self.invoice_date_due or self.invoice_reception_date or today
