...
class ResUser(models.Model):
    ...
    read_only = fields.Boolean(string="Aplicar Solo Lectura")
    
    @api.onchange('read_only')
    def set_read_only_user(self):
        read_only_grp_id = self.env['ir.model.data'].get_object_reference('access_read_only_user', 'group_read_only_user')[1]
...
class IrModelAccess(models.Model):
    ...
    def check(self, model, mode='read', raise_exception=True):
        ...
        if self.env.user.has_group('access_read_only_user.group_read_only_user'):
            if mode != 'read':
                return False
        return result
...
class IrRule(models.Model):
    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        if model_name not in obj_list:
            if self.env.user.has_group('access_read_only_user.group_read_only_user'):
                if mode != 'read':
                    raise ValidationError(_('El usuario: %s \nSolo tiene Acceso de Lectura') % self.env.user.name)
        return res
