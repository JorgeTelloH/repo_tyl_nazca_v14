...
class AccountMove(models.Model):
    ...
    payment_retention = fields.Selection([("percent", "Porcentaje"), ("amount", "Monto")],
        string="Tipo de Retención", readonly=True,
        states={"draft": [("readonly", False)]},
        help="Monto de retención sugerido que se retendrá en el pago.\n"
        "Nota: Durante el pago, el usuario puede no saberlo.",
    )
    amount_retention = fields.Float(string="Retención", readonly=True,
        states={"draft": [("readonly", False)]},
        help="Retención en porcentaje o monto de este Comprobante",
    )
    retention_amount_currency = fields.Monetary(string="Monto retención",
        compute="_compute_retention_amount_currency", store=True,
        help="Según el tipo de retención, este campo muestra la cantidad a retener.",
    )
    retention_residual_currency = fields.Monetary(string="Retención residual",
        compute="_compute_retention_residual_currency",
    )
    retained_move_ids = fields.Many2many(comodel_name="account.move",
        relation="account_invoice_move_rel", column1="invoice_id", column2="move_id",
        string="Devolución de Retención", readonly=True,
        states={"draft": [("readonly", False)]},
        copy=False,
    )
    ...
    def _onchange_domain_retained_move_ids(self):
        self.retained_move_ids = False
        domain = []
        if self.env.user.has_group("account_invoice_payment_retention.group_payment_retention"):
            dom = [
                ("parent_state", "=", "posted"),
    ...
    def _compute_retention_amount_currency(self):
        ...
        for rec in self:
            ...
            if rec.payment_retention == "amount":
                ...
                amount_untaxed = sum(rec.invoice_line_ids.filtered(lambda l: l.quantity > 0).mapped("amount_currency"))
        ...
        for rec in self.filtered("payment_retention"):
            if rec.retention_amount_currency > rec.amount_untaxed:
                raise ValidationError(
                    _("La retención no debe exceder del Total Base Imponible")
                )
