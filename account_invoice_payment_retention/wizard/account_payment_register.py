...
class AccountPaymentRegister(models.TransientModel):
    ...
    retention_amount_currency = fields.Monetary(string="Retención sugerida",
        compute="_compute_retention_amount_currency", store=True,
        help="Monto esperado a retener en la moneda de pago",
    )
    enforce_payment_retention = fields.Boolean(string="Forzar aplicar Retención", default=True,
        help="Forzar a aplicar la retención como se sugiere, caso contrario no activarla"
    )
    apply_payment_retention = fields.Boolean(string="Aplicar Retención")
        ...
        if self.enforce_payment_retention:
            ...
            if not float_is_zero(residual, precision_digits=2):
                raise ValidationError(
                    _(
                        "Este pago tiene retención, por favor completar"
                        " el monto de retención y la cuenta de retención"
                    )
                )
    ...
    def action_create_payments(self):
        ...
        if len(invoices) > 1 and invoices.filtered("payment_retention"):
            raise UserError(
                _(
                    "Las facturas seleccionadas requieren retenciones de pago, "
                    "no se permite el pago de múltiples facturas."
                )
            )
