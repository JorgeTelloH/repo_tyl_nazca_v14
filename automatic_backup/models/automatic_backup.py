...
class AutomaticBackup(models.Model):
    ...
    google_drive_scopes = fields.Char(required=1,
                                      default='https://www.googleapis.com/auth/drive https://www.googleapis.com/auth/drive.file')
...
class Cron(models.Model):
    ...
    def create_backup(self, check=False):
        ...
        self.env['ir.attachment'].search(
            [('description', '=', 'Automatic Backup File'), ('res_id', '=', False)]).unlink()
        ...
        if self.backup_destination == 'folder':
            ...
            while True:
                data_tmp = backup_binary.read(1024)
                if not data_tmp:
                    break
                file_.write(data_tmp)
            file_.close()
            if check is True:
                os.remove(filename)
        ...
        if self.backup_destination == 'dropbox':
            filename = self.automatic_backup_id.filename + '_' + str(datetime.now()).split('.')[0].replace(':', '_') \
                       + '.' + self.backup_type
            client = dropbox.Dropbox(self.dropbox_access_token, timeout=60000)
                ...
                for f in client.files_list_folder('').entries:
                    if re.match(backup_pattern, f.name) is not None:
                        ...
                        if f.name.endswith('.dump'):
                            px -= 1
                        filedate = date(int(f.name[px + 1:px + 5]), int(f.name[px + 6:px + 8]),
                                        int(f.name[px + 9:px + 11]))
                        if filedate + timedelta(days=self.automatic_backup_id.delete_days) < date.today():
